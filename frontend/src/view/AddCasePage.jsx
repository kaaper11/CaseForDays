...
const AddCasePage = () => {
    ...
    const addItem = (item, text) => {
        setAddedItems([...addedItems, item])
        // index.current.style.border = '1px solid red'
    }
    ...
    return (
        <div className={'flex flex-col items-center py-5'}>
            ...
            <form className={'flex flex-col gap-5 py-5 border-3 w-200 items-center rounded-lg border-cyan-800 shadow-2xl'}>
                ...
                <buttons className={'w-40 border-3 border-cyan-800 text-center rounded-md hover:bg-cyan-800 text-white cursor-pointer font-bold '}>Dodaj skrzynkę</buttons>
            </form>
            <div className="grid grid-cols-5 gap-5 py-10">
                {items.map((item, index) => (
                    <div  onMouseEnter={() => setOpis(true)} onMouseLeave={() => setOpis(false)} className={'skin w-60 h-80 flex items-center justify-center text-center flex-col gap-0  border-3 cursor-pointer hover:shadow-2xl rounded-md border-cyan-800'}>
                        <img src={item.image.split('\\').pop()}/>
                        <div className={'flex w-60 h-35 flex-col border-1 border-cyan-800'}>
                        ...
                        <div className={'flex w-60 h-35 items-center justify-center border-1 border-cyan-800'}>
